...
void pack_poly_m_R(uint8_t bytes[R_BYTES], poly s[m - D]) {
    int l;
 ...
 * @param bytes - input byte array
 */
void unpack_poly_m_R(poly s[m - D], const uint8_t bytes[R_BYTES]) {
    int l;
...
int get_sig_bytes(unsigned int in_len, unsigned int out_len) {
    return (((int) ceil(log2(TAU3 * (out_len + in_len))) + 1) * N)/8 * (m - D);
}
...
void pack_poly_sig(uint8_t bytes[], poly s[m - D], int additions) {
    int l;
    int SINGLE_sig_BYTES  = (((int) ceil(log2(TAU3 * additions)) + 1) * N)/8;
    for (l = 0; l < (m - D); l++) {
